...
# Test recognition
test_face = images[0]
scores = [model.score_samples(pca.transform(test_face.reshape(1, -1))) for model in hmms]

# Extract likelihood scores and corresponding subject labels
likelihoods = [score[0] for score in scores]
predicted_person = np.argmax(likelihoods) + 1

# Plot the likelihood scores
plt.figure(figsize=(10, 6))
plt.bar(range(1, 41), likelihoods, color='blue')
plt.xlabel('Subject')
plt.ylabel('Likelihood Score')
plt.title('Likelihood Scores for Face Recognition')
